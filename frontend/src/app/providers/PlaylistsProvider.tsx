...
export const PlaylistsProvider: React.FC<PlaylistsProviderProps> = ({ children }) => {
  ...
  const addTrackToPlaylist = useCallback(
    async (playlistId: string, trackId: string): Promise<boolean> => {
      ...
      try {
        ...
        console.error(`Error Error adding track ${trackId} to playlist ${playlistId}:`, error);
        return false;
      }
    },
    [fetchPlaylists, currentPlaylistId, currentPlaylistTracks.length]
  );
    ...
    async (playlistId: string, trackIds: string[]): Promise<{ successful: number; failed: number; errors: string[] }> => {
      ...
      for (const trackId of trackIds) {
        try {
          ...
          console.error(`Error Error adding track ${trackId} to playlist ${playlistId}:`, errorMessage);
  ...
  const updatePlaylistOrder = useCallback(async (playlistIds: string[]): Promise<Playlist[]> => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
      console.error(`Error Error updating playlist order:`, errorMessage);
      setError(errorMessage);
  ...
  const updatePlaylistTrackOrder = useCallback(async (playlistId: string, trackIds: string[]): Promise<boolean> => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error occurred';
      console.error(`Error Error updating track order for playlist ${playlistId}:`, errorMessage);
      setError(errorMessage);
      return false;
    }
  }, [currentPlaylistId]);
